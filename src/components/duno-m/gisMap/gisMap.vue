<template>
    <div class="gisMap">
        ...
        <div v-for="(item, index) in deviceList" :key="'device'+index" @dragstart="drag($event, item)" :style="'transform:rotate('+ item['direct'] +'deg)'"  @click="toDeviced(item,index,null,1)" v-show="item['show']" class="anchorList" :id="'anchor'+index" ><img :src="item['src']" :style="{cursor: cursor}" alt="示例锚点"/></div>
        <div v-for="(item, index) in powerPointList" :key="'powerPoint'+index"  @click="toDeviced(item,index,null,1)"  class="anchorList" :id="'anchord'+index" ><img style="width: 5px ;height: 5px" :style="{cursor: cursor}" :src="item['src']" alt="示例锚点"/></div>
