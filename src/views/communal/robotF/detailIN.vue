<template>
  <div class="robotDetail">
    ...
    <div class="table">
      ...
      />
      <el-pagination
        v-show="specialInspectList && specialInspectList['data']"
    ...
    <robot-task v-if="dialogVisible" :isShow='dialogVisible'  @on-close="onClose" />
...
export default {
  ...
  data() {
    ...
    return {
      ...
      dataBread: [
        ...
        { path: "/robot-two/list", name: "室内轨道机器人" },
        { path: "", name: "任务配置列表" }
      ],
      newColumns: [
        {
  ...
  methods: {
    ...
    getTableData() {
      const that = this;
      postAxiosData("/lenovo-robot-indoor/rest/taskNormalDetail", {
        taskId: that.taskId
      }).then(res => {
        let data = res.data;
    ...
    initData() {
      ...
      let substationId = this.$route.query.substationId;
      let robotId = this.$route.query.robotId;
      postAxiosData("/lenovo-robot-indoor/rest/tasks", {
    ...
    getInfor() {
      let substationId = this.$route.query.substationId;
      let robotId = this.$route.query.robotId;
      getAxiosData("/lenovo-robot-indoor/rest/specialTasks", {
        substationId: substationId,
        robotId: robotId,
